        ...
        for (const method in ajvPathSchema) {
            ...
            for (const type in ajvMethodSchema) {
                ...
                const interfaceFileName = interfaceName
                    ...
                    .substr(2);
                const validationJsonFile = resolve(`validations/${interfaceFileName}.json`);
                const validationDtsFile = resolve(`validations/${interfaceName}.d.ts`);
                await unlink(validationJsonFile);
                await unlink(validationDtsFile);
                await writeFile(validationJsonFile, JSON.stringify(ajvTypeSchema, null, 2));
                await new Promise((resolve, reject) => exec(`yarn json2ts -i ${validationJsonFile} -o ${validationDtsFile}`, (error, stdout, stderr) => {
                    if (!error && !stderr) {
                        resolve(stdout);
                    }
                    reject(error || stderr);
...
export { swaggerParse as default };
//# sourceMappingURL=swagger-parse.esm.js.map
